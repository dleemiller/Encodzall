...
def random_contiguous_sublist(lst: list[str], length: int) -> list[str]:
    """
    Selects a random contiguous sublist of a fixed length from a given list.
    """
    if len(lst) < length:
        raise ValueError(
            f"The list length ({len(lst)}) is shorter than the desired sublist length ({length})."
        )
